...
function compare(a, la, b, lb) {
    if (la > lb) {
        return compare(a.next, la - 1, b, lb);
    }
    if (la < lb) {
        return compare(a, la, b.next, lb - 1);
    }
    if (a == b) {
        return a;
    }
    return compare(a.next, la - 1, b.next, lb - 1);
}
var getIntersectionNode = function(headA, headB) {
    if (!headA || !headB) return null;
    let l1 = getLength(headA);
    let l2 = getLength(headB);
    return compare(headA, l1, headB, l2);
};
